...
protected:
    ...
    MicroOrganism(int a, float lngth){ // sadece squadratic cagiriyor
        for(int i=0; i<4; i++){
            cell_wall.push_back(new Wall(lngth));
        }
        id=a;
        located=false;
        attached_cell=NULL;
    }
    MicroOrganism(float radiuss){  //sadece circular cagiriyor
       cell_wall.push_back(new CurvyWall((float) 2*PI*radiuss,radiuss)); //cevresi
        located=false;
        attached_cell=NULL;
    }
